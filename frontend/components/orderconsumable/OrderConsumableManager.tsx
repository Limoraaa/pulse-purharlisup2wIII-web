...
export default function OrderConsumableManager() {
  ...
  const fetchOrders = async (status: string = statusFilter) => {
    ...
    setError(null);
    try {
      const data = await getOrderConsumables(status);
      setOrders(data || []);
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteOrderConsumable(deleteId);
        ...
        <CardBody>
          ...
          {loading ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-secondary mb-0">Belum ada pengajuan order consumable yang tercatat.</p>
            </div>
          ) : (
